...
const storage = multer.diskStorage({
  destination: function (req, file, cb) {
    ...
    if (fieldName.startsWith('files[') && fieldName.endsWith(']')) {
      ...
      const fullDirPath = path.join(__dirname, '../../', 'static', 'fund_amain', dirPath);
...
//
      ...
      cb(null, path.join(__dirname, '../../', 'static', 'fund_amain'));
    }
  },
...
// 处理任意字段名的文件
router.post('/fund_apifolder_fundamain2222',
  upload.any(), // 处理所有字段
  (req, res) => {
    // 设置超时机制
    ...
    const uploadPromise = new Promise((resolve, reject) => {
      try {
        const managePath = path.join(__dirname, '../../', 'static', 'fund_amain');
...
// 处理任意字段名的文件
router.post('/fund_apifolder_fundamain',
  (req, res, next) => {
    try {
      const managePath = path.join(__dirname, '../../', 'static', 'fund_amain');
      // 清空目录
