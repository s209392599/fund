    ...
    return limit(async () => {
      ...
      try {
        ...
        if (datas) {
          ...
          return {
            ...item,
            annual_return: parseFloat(rate_12), // 添加年化收益字段
            // established_date: establishedDate, // 成立日期
            // fund_scale: fundScale, // 基金规模
            // rank_1: rank_1, // 近1月排名
            // rank_3: rank_3, // 近3月排名
            // rank_6: rank_6, // 近6月排名
          };
...
async function main() {
  ...
  try {
    ...
    // 按年化收益从高到低排序
    info.filter_data.sort((a, b) => (b.annual_return || 0) - (a.annual_return || 0));
