...
export default {
  ...
  created() {
    this.tableData = localStorage.getItem('selftFundInfo')
  ...
  methods: {
    ...
    importDataGuokun() {
      ...
      localStorage.setItem('selftFundInfo', JSON.stringify(arr));
    ...
    copyData() {
      ...
      let str = `var arr =${localStorage.getItem(
        'selftFundInfo'
      )};localStorage.setItem("selftFundInfo",JSON.stringify(arr));window.location.reload();`;
      this.copy(str);
    },
    addInfo() {
      //添加一条信息
    ...
    handleDelete(index) {
      ...
      localStorage.setItem('selftFundInfo', JSON.stringify(this.tableData));
    },
    editSure(formName) {
      // 模态框 确定事件
