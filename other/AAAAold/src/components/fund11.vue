  ...
  <div class="about" ref="boxue" id="boxue">
    <template v-for="(item, index) in fundArr" :key="index">
      <div
        :ref="'chartEl_' + index"
        :id="'ChartBox_' + index"
        style="float: left; width: 352px; height: 277px"
      ></div>
    </template>
  </div>
</template>
...
export default {
  ...
  created() {
    this.fundArr = localStorage.getItem('info')
  ...
  methods: {
    getShouyi(params) {
      setTimeout(() => {
        axios
          .get(`${this.fundURL}/obtainNetWorth/?code=${params.number}&size=780`)
          .then((res) => {
            let data = res.data || [];
            let totalNetValue = data
              .reverse()
              .map((item) => item.totalNetValue);
    ...
    Render_Chart(text = '', data = [], index = 0) {
      const chartBox = echarts.init(
        document.getElementById('ChartBox_' + index)
      );
