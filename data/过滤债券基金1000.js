...
async function getPageMutilDataNotLogin() {
  let productList = [];
  try {
    let u = `https://fund.eastmoney.com/data/fundtradenewapi.aspx?ft=zq&sc=1n&st=desc&pi=1&pn=1000&cp=&ct=&cd=&ms=&fr=&plevel=&fst=&ftype=&fr1=&fl=0&isab=1`;
...
async function isEligible(arr) {
  ...
  for (let index = 0; index < arr.length; index++) {
    console.log(
      ...
      }--已完成的基金数--${EligibleList.length}/${arr.length}`
    );
